...
public class GoRest4 {
	...
	public void initialize() {
		...
		String reportName="./Reports/goRest"+d1+".html";
		
		spark = new ExtentSparkReporter(new File(reportName));
		report = new ExtentReports();
		report.attachReporter(spark);
		spark.config().setDocumentTitle("GoRestAPITestingWithDate");
		spark.config().setTheme(Theme.DARK);
		spark.config().setReportName("GoRest4");
		
		report.setSystemInfo("Environment", "TestEnv");
		report.setSystemInfo("TesterName", "chris");
	}
	@Test(priority =1)
